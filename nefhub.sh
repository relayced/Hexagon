...
write_file() {
    if command -v cat >/dev/null 2>&1; then
        cat > "$1"
    elif [ -x /system/bin/cat ]; then
        /system/bin/cat > "$1"
    elif [ -x /system/bin/toybox ]; then
        /system/bin/toybox cat > "$1"
    elif [ -x /system/bin/toolbox ]; then
        /system/bin/toolbox cat > "$1"
    else
...
const (
	ScriptVersion = "1.4.2"
	VersionURL    = "https://raw.githubusercontent.com/kameskill/autorejoin/main/version.txt"
